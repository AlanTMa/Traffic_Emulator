...
def test_non_binding_example_from_paper_formula():
    ...
    assert compute_safe_step(lam, br, loads, mu, eta=0.25, eps=0.0) == pytest.approx(1.0)
...
def test_binding_single_broker():
    ...
    s = compute_safe_step(lam, br, loads, mu, eta=0.25, eps=1e-8)
...
def test_min_over_brokers_ignores_decreasing_loads():
    ...
    assert compute_safe_step(lam, br, loads, mu, eta=eta, eps=0.0) == pytest.approx(min(s_0, s_1))
...
def test_iteration_step_binding_matches_algorithm_1():
    ...
    eta, gamma, eps = 0.25, 0.01, 1e-8
    state = SystemState(lambda_ij=lam0.copy(), prices=prices0.copy())
    state, s_t, _ = iteration_step(state, topo, eta, gamma, eps)
    ...
    s_j = [min(1.0, (topo.mu_brokers[j] - eps - loads[j]) / (eta * delta[j])) if delta[j] > 0 else 1.0
           for j in range(2)]
    s_expected = min(s_j)
    ...
    # Broker capacity: the binding broker lands on mu - eps, never beyond
    assert np.all(new_loads <= topo.mu_brokers - eps + 1e-9)
    assert new_loads[0] == pytest.approx(topo.mu_brokers[0] - eps, abs=1e-9)
    # Source conservation
...
def test_binding_step_stays_feasible_over_iterations():
    ...
    state = SystemState(lambda_ij=lam0.copy(), prices=prices0.copy())
    for _ in range(200):
        state, s_t, _ = iteration_step(state, topo, 0.25, 0.01, 1e-8)
