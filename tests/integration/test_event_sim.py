...
def test_event_driven_mm1_latency():
    ...
    sources = ["P0"]
    brokers = ["SN1"]
    lambdas_total = np.array([0.9]) # 0.9 pkts/s - High load to make queueing obvious
    ...
    engine.run(duration=2000.0, handler=handler.handle_event)
    ...
    # = 1/(1-0.9) + 1/(1-0.9) = 10 + 10 = 20.0
    expected = 20.0

    print(f"\nSimulated Mean Latency: {mean_latency:.4f}")
    print(f"Expected Mean Latency: {expected:.4f}")

    # Allow 20% margin for stochastic variance
    assert mean_latency == pytest.approx(expected, rel=0.2)
